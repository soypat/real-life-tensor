...
Rf = 20;
Rp = Rb + Rf;
Rho_p = (((Rp + s).^2 + v.^2).^(3/2))./((Rp + s).^2 + 2*v.^2 - a.*(Rp + s));

while ~all(abs(Rho_p)>Rf)
   Rb = Rb + 1; 
   Rp = Rb+Rf;
   Rho_p = (((Rp + s).^2 + v.^2).^(3/2))./((Rp + s).^2 + 2*v.^2 - a.*(Rp + s));
end
...
Rp = Rb + Rf;
[paso_rodillo,circulo_de_base,circulo_primario] = rodimatic(s,Rf,Rb);
levard = levamatic(paso_rodillo,Rf);%Estos valores para graficar
%Determinación de epsilon

e = -1;

phi = atan((v-e)./(s + (Rp^2 - e^2)^(1/2))).*(180/pi);
...
for e = -4:1:4
    phi = atan((v-e)./(s + (Rp^2 - e^2)^(1/2))).*(180/pi);
    phia = abs(phi);
    phimaxa(e+5) = max(phia);
end
plot(-4:4,phimaxa);
grid on
grid minor
title('Modulo de Angulo de presión vs excentricidad','fontsize',14)
xlabel('Excentricidad[mm]','fontsize',14)
ylabel('Maximo valor absoluto de Phi[°]','fontsize',14)
disp(phimax);
disp(phimin);
