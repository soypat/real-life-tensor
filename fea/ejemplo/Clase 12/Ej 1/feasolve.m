%% Práctica 12, ej 1
%clear; close all; clc
function [Error] = feasolve(eleT,nodes,elements)
%% Datos
c = 10;
L = 100;
w = 1;
...
switch eleT
    ...
    case 'Q8'
        nodes = load('nodQ8.txt');
        elements = load('eleQ8.txt');
        elements = elements(:,2:9);
    case 'Q9'
    case 'CST'
        nodes = load('nodCST.txt');
        elements = load('eleCST.txt');
        elements = elements(:,2:4);
    case 'LST'
        nodes = load('nodLST.txt');
        elements = load('eleLST.txt');
