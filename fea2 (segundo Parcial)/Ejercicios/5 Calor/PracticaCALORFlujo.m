...
%         for inode = 1:nNodEle
%         %flujo por conduccion, preguntar??    
%             if eleDofst(inode)==5||eleDofst(inode)==10
%                 Rc()=Rc()-N'*N*wpg(ipg)*jac(2,2)*[0 0.625 0.625 0]';
%             end
%         end
%         
        A = A + wpg(ipg)*Djac;
...
Tc([1,2])=100;%los nodos con respecto a Co
Rx=Kxc*Tc;
Rc([4,8])=[-0.625 -0.625*2];
Rx=Rc+Rx;
Tx=Kxx\-Rx;
T=zeros(nNod,1);
T(X)=Tx;
...
for iele = 1:nel
    ...
    for inode = 1:nNodEle
        ...
        Bt=dNxy;
        eleDofs = node2dof(elementos(iele,:),nDofNod);
        stress(inode,:,iele) = C*(B*D(eleDofs)-(E*alfa*TnodosEle(inode)*[1 1 0]')/(1-2*nu));
