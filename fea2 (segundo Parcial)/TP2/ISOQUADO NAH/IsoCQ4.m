...
nDofNod = 1;                    % grados de libertad por nodo
...
bc = false(15,1);       % Matriz de condiciones de borde
bc(2:5,1:2) = true;
%%
% figure(1)
...
%% Cargas
Co=[10,5,1,6,11,12 13,14,15];
X=zeros(1,nNod-length(Co));
aux=1;
...
indexcien=[find(Co==1) find(Co==6)];%Le meto 100 grados a mis nodos 6 y 0
Tc(indexcien)=100;%Asigno un valor a esos nodos conocidos
Q=Kxc*Tc;
Tx=Kxx\-Q;
T=zeros(15,1);
T(X)=Tx;
